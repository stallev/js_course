...
export interface UserContentProps {
  ...
  albumStats: UserAlbumStats | null;
}
...
export async function fetchUsers(): Promise<PlaceholderUser[]> {
  const res = await fetch(`${PLACEHOLDER_API}/users`);
  return parseJson(res, `fetchUsers()`);
}

export const searchUsers = async (query: string): Promise<PlaceholderUser[]> => {
  const res = await fetch(`${PLACEHOLDER_API}/users?name_like=${query}`);
  return parseJson(res, `searchUsers(${query})`);
}