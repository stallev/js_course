...
Навигация по всем темам курса. Разделы 1–8 — JavaScript **под капотом** и паттерны (несколько тем в одном файле). Разделы 9–14 — React и библиотеки управления состоянием (каждая тема в отдельном файле).

**Вход:** уже пишешь на React / TypeScript и готовишь интервью. Азбука языка — [`junior/`](../../junior/README.md); азбука типов без React — [`typescript/`](../../typescript/README.md). Тема 24 здесь не заменяет мост. Если `let` и `fetch` ещё неуверенны — не начинай с Event Loop.
